...
export async function POST(request: Request) {
    let body: { restaurantId?: string; address?: string; phoneNumber?: string; directionsUrl?: string }
    try {
    ...
    try {
        ...
        const result = await owners.updateOne(
            { restaurantId },
            { $set: { address, phoneNumber, directionsUrl, updatedAt: new Date() } }
        )
        ...
        return NextResponse.json({ success: true, address, phoneNumber, directionsUrl })
    } catch (error) {
